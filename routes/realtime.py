...
def incr_bot_error(dbs, botid, bot, error):
    ...
    error_map = {'insufficient-balance': 'Insufficient balance on exchange to make trade', 'failed-exchange-auth': 'API Key/Authentication was rejected by exchange'}
    ...
    if len(errs) > errors_limit:
        def fmtdate(t): return datetime.datetime.fromtimestamp(t).strftime("%d/%b/%Y %X UTC")
...
"""
    ...
    dbs['globals'].set(err_key, errs, expire=7 * 24 * 24 * 60)
...
def handle_failed_auth(data):
    ...
    username = bot['user']
    if int(time.time()) - dbs['globals'].get(f"{username}:{bot['exchange']}:failed-auth", 0) < 2 * 24 * 60 * 60:
        return
    ...
    """
    notification = Notification("Failed to authenticate at exchange", body, 'system', int(time.time()), icon='error')
    NotificationSaving(dbs).save_notification(notification, username)
    dbs['globals'].set(f'{username}:{bot["exchange"]}:failed-auth', int(time.time()), expire=7 * 24 * 60 * 60)

    NotifyMailPreferences(dbs).notify_mail_preferences(username, 'trade:failed-exchange-auth', param={
        'exchange': bot['exchange']
    })
...
def handle_failed_trade(botid, data):
    ...
    username = bot['user']
    strat_meta = StrategyFactory(bot['strategy'], dbs)
    if int(time.time()) - dbs['globals'].get(f'{username}:{bot["exchange"]}:insufficient-balance', 0) < 24 * 60 * 60:
        return
    ...
    dbs['globals'].set(f'{username}:{bot["exchange"]}:insufficient-balance', int(time.time()), expire=7 * 24 * 60 * 60)

    NotifyMailPreferences(dbs).notify_mail_preferences(username, 'trade:insufficient-balance', param={
        'market': data['market'],
...
def handle_trade_filled(dbs, bot, botid, info):
    def rd(x): return round(float(x), 5)
    strat_meta = StrategyFactory(bot['strategy'], dbs)
    ...
    try:
        info['OBT_fee'] = OBTokenTransaction(dbs).deduct_bot_trade_fee(email=bot['user'], amount=info['amount']*info['price'],currency=cur,bot=bot).ok() or 0
...
<b>Change since last trade:</b> {'' if change < 0 else '+'} {rd(change)} %
    """
    notification = Notification(title, body, 'trade', int(time.time()), icon=tok)
    NotificationSaving(dbs).save_notification(notification, bot['user'])

    __total_balance = (rd(info['amount']) + rd(bot['state'].tokBalance))
    __total_balance = 0 if __total_balance == 0 else rd(info['amount'])/__total_balance
    ...
    if len(bot.get('twitter_tokens', []) or []) == 4:
        notificationHandler.post_tweet(bot['twitter_tokens'][0:2], bot['twitter_tokens'][2:4], notificationHandler.twitter_message_template(
            message=message
        ))

    if bot.get('telegram_sent_username', False) and bot.get('telegram_token', False):
        notificationHandler.notify_telegram(telegram_token=bot['telegram_token'],
                                            telegram_sent_username=bot['telegram_sent_username'],
                                            text=notificationHandler.telegram_message_template(message=message),
                                            username=bot['user'])
...
def handle_stoploss_triggered(dbs, bot, botid, info, env):
    ...
    for order in list(open_orders):
        # Cancel on the exchange, and mark as such on our side
        try:
            env.get_api().cancel_order(profile['exchanges'][bot['exchange']], bot['market'], open_orders[order].txid)
    ...
    roi = (env.portfolioValue() / ((bot['state'].startingBalance[0] * bot['starting_price']) + bot['state'].startingBalance[1]))-1
...
<b>Ending ROI: {rd(roi) * 100}%</b><br>
    """
    notification = Notification(title, body, 'system', int(time.time()), icon=tok)
    NotificationSaving(dbs).save_notification(notification, bot['user'])
    bot['enabled'] = False
    bot['stop_time'] = time.time()

    NotifyMailPreferences(dbs).notify_mail_preferences(bot['user'], 'trade:stoploss', param={
        'market': bot['market'],
        'cur': cur,
        'bot type': strat_meta.get_name(),
        'token price': rd(info['price']),
        'body': f''' On {date:%d-%m-%Y %H:%M:%S} UTC  your {bot['market']} {strat_meta.get_name()} on {bot['exchange']} has been archived as it's stop-loss had triggered. 
...
def handle_with_clientOrderId_fail(dbs,bot,botid,cid,err_type,err_ctx):
    if not dbs['globals'].get("failed:clientOrders"):
        dbs['globals'].get["failed:clientOrders"]=dict()
...
def realtime_thread(exchange: str, exchange_cfg: dict):
    ...
    logger = getLogger(f'realtime.{exchange}')
    dbs = get_dbs()
    for candle in exchange_cfg['candles']:
        for market in exchange_cfg['pairs']:
            ...
            for k in dbs['bots']:
                ...
                if bot['market'] != market or candle != bot['candles'] or not bot['enabled'] or bot['exchange'] != exchange:
                    continue

                total += 1

                # Setup event handling
                fhandle_trade_filled = partial(handle_trade_filled, dbs, bot, k)
                fhandle_stoploss_triggered = partial(handle_stoploss_triggered, dbs, bot, k)
                fhandle_failed_trade = partial(handle_failed_trade, k)

                fhandle_with_clientOrderId_fail = partial(handle_with_clientOrderId_fail,dbs=dbs,bot=bot,botId=k)
                ...
                env.once('trade:clientOrderId-fail', fhandle_with_clientOrderId_fail)
                ...
                try:
                    ...
                    lllogger.info(f"Running {type(strat).__name__}: {bot['state'].portfolioValue} <- tok: {bot['state'].tokBalance}, cur: {bot['state'].curBalance}")
                    env.nstep()
                    ...
                    if bot['enabled']:
                        if new_candles:
                            ...
                            dbs['bots'][k] = bot
                            lllogger.debug(f"Raw model output: action={action}")
                        ...
                        if (bot.get('stop_loss') or {}).get('stop'):
                            ...
                            bot['stop_loss']['starting_portfolio'] = bot['stop_loss'].get('starting_portfolio') or bot['state'].startingBalance[1]

                            if bot['stop_loss'].get('trailing', False):
                                # Track highest portfolio since enabling stoploss
                                current_portfolio = env.portfolioValue()
                                if bot['stop_loss']['highest_portfolio'] < current_portfolio:
                                    bot['stop_loss']['highest_portfolio'] = current_portfolio

                                top_roi = bot['stop_loss']['highest_portfolio'] / bot['stop_loss']['starting_portfolio']
                                stop_loss_frac = top_roi * bot['stop_loss']['stop']
                            else:
                                stop_loss_frac = bot['stop_loss']['stop']

                            # Take into account the balance in orders for the price, but not for the actual order
                            cur, tok = bot['state'].curBalance, bot['state'].tokBalance
                            ...
                            if stop_loss_frac * bot['stop_loss']['starting_portfolio'] > cur:
                                ...
                                stop_price = ((stop_loss_frac * bot['stop_loss']['starting_portfolio']) - cur) / (tok + 1e-8)
                                if env.current_v() < stop_price:
                                    # Already below target price, just stop the bot
                                    tx = env.sell(0.99)
                                    env.emit('trade:stoploss', tx, env)
                                else:
                                    env._stop_order(bot['state'].tokBalance * 0.99, stop_price)

                    bot['bah_roi'] = (env.current_v() - bot['starting_price']) / bot['starting_price']
                    bot['state'].last_trade_attempt = time.time()

                except SystemExit:
                    return
                except Exception:
                    lllogger.warn('Failed to run bot')
                    traceback.print_exc(file=sys.stdout)
                finally:
            ...
            llogger.info('RT bots candles: %d, ran %d/%d bots' % (new_candles, ran, total))
...
class RealTimeEvl:
        ...
        for exchange in self.exchanges:
            for market in self.exchanges[exchange]['pairs']:
                for candle in self.exchanges[exchange]['candles']:
                    ...
                    if os.path.isfile(fpath):
                        try:
                            print(f'Checking enviroment for {exchange} {market} {candle}')
                            with open(fpath, 'rb') as f:
                            ...
                            print('Integrity check failed, rebuilding enviroment')

                    print(f'Downloading {exchange} {market} {candle}')
                    try:
                        mi = MarketInfo.from_download(market, candle, exchange=exchange)
                        rte = RealTimeEnv(mi)
                        # Store permanently
                        with open(fpath, 'wb+') as f:
                            pickle.dump(rte, f)
    ...
    def is_reload_env(self):
        if self.dbs['globals'].get('RealTimeEvl:is_reload_env', False):
            ...
            self.load_envs()
            self.dbs['globals'].set(key='RealTimeEvl:is_reload_env', value=False)

    def loop(self):
        with Pool(os.environ.get("RT_THREADS", 4)) as p:
            p.starmap(realtime_thread, [(x, self.exchanges[x]) for x in self.exchanges])
        self.is_reload_env()
